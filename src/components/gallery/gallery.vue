  ...
  <v-row justify="center" no-gutters>
    <v-col cols="12" lg="12">
      <div class="bg-primary py-12">
        <h2 class="text-h4 text-secondary text-center">Gallery</h2>
      </div>
    </v-col>
    <v-overlay v-model="overlay"  style="z-index: 55555555555555555" opacity="0.9">
      ...
      <v-row no-gutters style="height: 100vh;width: 100vw;flex-wrap: nowrap" justify="space-between" align="center">
        ...
        <v-col cols="12" lg="11">
          <gallery-overlay-item @nextImage="nextImage" @previousImage="previousImage" :image="imageOverlay"></gallery-overlay-item>
        </v-col>
    ...
    <v-col cols="12" lg="8" class="mt-8">
      <div class="grid-wrapper" ref="gallery">
        <template v-for="(image, i) in images" v-if="galleryWidth">
          <gallery-item :image="image" @openGallery="openGallery(i)" :width="galleryWidth"></gallery-item>
        </template>
      </div>
    </v-col>
  </v-row>
...
export default {
  ...
  methods: {
    previousImage() {
      if (!this.imageIndex) this.imageIndex = this.images.length - 1;
